...
class PreprocessorGUI:
    ...
    def _on_load(self):
        # Use tkinter file dialog as Open3D's FileDialog can crash on some platforms.
        try:
        ...
        # Fallback to Open3D FileDialog
        try:
            dlg = gui.FileDialog(gui.FileDialog.OPEN, "Open mesh", self.window.theme)
            dlg.add_filter("Mesh files", ["*.ply", "*.stl", "*.obj", "*.off", "*.gltf", "*.glb"])
            ...
            _log_exc("Open3D FileDialog fallback failed")
            _show_error_dialog(self.window, "File Dialog Error", "Unable to open file dialog. See gui_log.txt")
    ...
    def _load_mesh_from_path(self, path):
        ...
        def worker_load():
            try:
                ...
                target = float(self.target_len.get_value())
                iters = int(self.iter_edit.get_value())
    ...
    def _on_flatten(self):
        ...
        def worker():
            try:
                uv, uv_flat, VT = flatten_mesh_arap(self.v, self.f)
                # realign
                ...
                vertex_strain = calculate_vertex_strain(self.v, self.f, uv, uv_flat_aligned)
